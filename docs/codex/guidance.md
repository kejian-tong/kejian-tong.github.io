...
1. Treat this as a statically exported Next.js App Router site. Do not introduce features that require a Node.js runtime on the deployed site.
2. Use `src/config/site.ts` as the single source of truth for canonical URLs, shared metadata, core identity fields, and primary social profiles.
3. Keep `src/content/personal.ts` and `src/content/contacts.ts` aligned with `src/config/site.ts`. Do not reintroduce duplicated hard-coded contact values in multiple files.
...
5. Prefer updating typed data files under `src/content/` over scattering content strings across components.
6. Keep metadata consistent across pages: canonical URLs should use the production domain and shared helpers.
7. Use Node 20 LTS for local work. Run `nvm use` before validation; current tooling expects Node 20.19 or newer.
...
Use `npm run validate` for the standard type-check, lint, and build sequence.
